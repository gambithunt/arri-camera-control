...
function createMockAppStateStore() {
  ...
  return {
    ...
    setError: (error: string | null) => {
      update(state => ({ ...state, error, loadingState: { ...state.loadingState, error } }));
    },
...
function createMockPlaybackStore() {
  ...
  return {
    ...
    setPlaybackMode: (isInPlaybackMode: boolean, enteringPlaybackMode: boolean) => {
      console.log(`Mock: Setting playback mode - inPlayback: ${isInPlaybackMode}, entering: ${enteringPlaybackMode}`);
      update(state => ({
        ...state,
        isInPlaybackMode,
...
export const mockCameraApi = {
  ...
  setWhiteBalance: async (kelvin: number, tint?: number) => {
    console.log(`Mock: Setting white balance to ${kelvin}K${tint !== undefined ? `, tint: ${tint}` : ''}`);
...
import { shouldUseMockStores, shouldUseMockApi, shouldEnableDebugLogs } from '$lib/config/appConfig';

// Safe store access function
export function safeStoreAccess() {
  ...
  if (useMockStores) {
    return {
      ...
      cameraApi: useMockApi ? mockCameraApi : mockCameraApi, // TODO: Load real API when useMockApi is false
      isUsingMocks: true
    };
  }
  
  // Production mode - try to load real stores
  try {
    // TODO: Import real stores when they're available
    // const realStores = await import('$lib/stores');
    // const realApi = await import('$lib/api/cameraApi');
    
    if (enableDebugLogs) {
      console.log('🚀 Production mode - real stores not yet implemented, using mocks as fallback');
    }
    ...
    return {
      ...
      isUsingMocks: true // Will be false when real stores are implemented
    };
  } catch (error) {
    if (enableDebugLogs) {
      console.warn('⚠️ Failed to load real stores, falling back to mocks:', error);
    }
